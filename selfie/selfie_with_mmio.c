...
// -----------------------------------------------------------------
// ------------------- FRAMEBUFFER (SDL2) SUPPORT ------------------
// -----------------------------------------------------------------

// Include SDL2 BEFORE any other selfie declarations to avoid conflicts
#ifdef __linux__
#define _GNU_SOURCE
#endif

// Include stdint.h to properly define uint64_t when using system libraries
#include <stdint.h>
...
// Undefine macros that conflict with selfie's definitions
#undef UINT64_MAX
#undef INT64_MAX
#undef INT64_MIN

// Framebuffer MMIO constants
static const uint64_t FB_START_ADDR = 0x80000000;      // Start address of framebuffer in virtual memory
static const uint64_t FB_WIDTH_PIXELS = 320;           // Width in pixels
static const uint64_t FB_HEIGHT_PIXELS = 240;          // Height in pixels
static const uint64_t DRAW_REGISTER_ADDR = 0x90000000; // Control register to trigger refresh
#define FB_SIZE (FB_WIDTH_PIXELS * FB_HEIGHT_PIXELS * 4) // Size in bytes (RGBA)

// SDL variables
SDL_Window *sdl_window = NULL;
SDL_Renderer *sdl_renderer = NULL;
SDL_Texture *sdl_texture = NULL;
uint32_t *framebuffer_data = NULL; // Host-side framebuffer storage
uint64_t framebuffer_initialized = 0;
...
uint64_t SYSCALL_LOCK_INIT = 218;    // Locks
uint64_t SYSCALL_LOCK_ACQUIRE = 219;
uint64_t SYSCALL_LOCK_RELEASE = 220;
...
// +----+-----------------+
// | 35 | held_locks_head | pointer to first held lock (Lockdep)
// | 36 | held_locks_count| number of locks held (Lockdep)
// +----+-----------------+

// number of entries of a machine context:
// 14 uint64_t + 6 uint64_t* + 1 char* + 7 uint64_t + 2 uint64_t* + 4 uint64_t entries
// extended in the symbolic execution engine, the Boehm garbage collector, and Lockdep
...
// lock_struct
// +---+--------------------+
// | 0 | semaphore_id      | ID del semáforo binario
...
void set_lock_semaphore(uint64_t *lock, uint64_t sem_id) { *(lock) = sem_id; }
...
void print_dependency_chain(uint64_t from, uint64_t to)
{
    ...
    // Use parameters to suppress warnings (will be optimized away)
    from = from;
    to = to;
...
uint64_t lockdep_lock_acquire(uint64_t *context, uint64_t lock_class)
{
    ...
    while (held != (uint64_t *)0)
    {
        ...
        // Ignore self-loops (acquiring same lock recursively)
        // This is not a deadlock scenario
...
void init_lockdep()
{
    ...
    // Silent initialization - no output to avoid interfering with tests
...
uint64_t *used_locks = (uint64_t *)0; // Locks: Array of used locks

uint64_t id_ctxt_counter = 0; // fork
uint64_t next_sem_id = 0; // Semaphores: Semaphore id counter
uint64_t next_lock_id = 0; // Locks: Lock id counter
...
void implement_fork(uint64_t *context){ // fork
  ...
  set_exception(child_context, get_exception(context));
  set_fault(child_context, get_fault(context));
  set_exit_code(child_context, get_exit_code(context));
...
void implement_sem_wait(uint64_t* context) {
  ...
  // Check for potential deadlock before acquiring semaphore
  sem_class = sem_addr;
  can_acquire = lockdep_lock_acquire(context, sem_class);

  if (can_acquire == 0) {
    // Deadlock detected - deny acquisition and advance PC
    set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
    return;
  }
...
void implement_sem_post(uint64_t* context) {
  ...
  // Notify lockdep that semaphore is being released
  sem_class = sem_addr;
  lockdep_lock_release(context, sem_class);
...
void init_framebuffer()
{
  ...
  sdl_window = SDL_CreateWindow(
      "Selfie Framebuffer MMIO",
      SDL_WINDOWPOS_UNDEFINED,
      SDL_WINDOWPOS_UNDEFINED,
      FB_WIDTH_PIXELS * 2, // Scale 2x for better visibility
      FB_HEIGHT_PIXELS * 2,
      SDL_WINDOW_SHOWN);
  ...
  // Allocate host framebuffer storage
  framebuffer_data = (uint32_t *)malloc(FB_SIZE);
  ...
  // Initialize to black
  memset(framebuffer_data, 0, FB_SIZE);

  framebuffer_initialized = 1;
  printf("Framebuffer initialized: %dx%d\n", (int)FB_WIDTH_PIXELS, (int)FB_HEIGHT_PIXELS);
}
...
void refresh_framebuffer()
{
  ...
  SDL_UpdateTexture(sdl_texture, NULL, framebuffer_data, FB_WIDTH_PIXELS * sizeof(uint32_t));
...
uint64_t do_store()
{
  ...
  // *** MMIO FRAMEBUFFER INTERCEPTION ***
  // Check for DRAW_REGISTER write (trigger refresh)
  ...
  // Check for framebuffer write (FB_START to FB_START + FB_SIZE)
  if (vaddr >= FB_START_ADDR && vaddr < FB_START_ADDR + FB_SIZE)
  {
    ...
    uint64_t offset = vaddr - FB_START_ADDR;
    uint64_t pixel_index = offset / 4; // 4 bytes per pixel (RGBA)
    
    if (pixel_index < FB_WIDTH_PIXELS * FB_HEIGHT_PIXELS && framebuffer_data != NULL)
    {
      // Store pixel data in framebuffer
...
uint64_t handle_system_call(uint64_t *context)
{
  ...
  else if (a7 == SYSCALL_LOCK_INIT) // locks
    implement_lock_init (context);
  else if (a7 == SYSCALL_LOCK_ACQUIRE) // locks
    implement_lock_acquire (context);
  else if (a7 == SYSCALL_LOCK_RELEASE) // locks
    implement_lock_release (context);
  else if (a7 == SYSCALL_EXIT)
  {
    implement_exit(context);

    // Mark context as exited by setting blocked flag to special value
    set_blocked(context, 2); // 0=ready, 1=blocked, 2=exited
...
uint64_t selfie_run(uint64_t machine)
{
  ...
  init_memory(selfie_atoi(peek_argument(0)));
  ...
  used_locks = smalloc (sizeof (uint64_t) * LOCKENTRIES * 512); // locks: initialize lock table
...
uint64_t selfie_run_mipsterOS(uint64_t machine){ // process_lab
  ...
  num_contexts = selfie_atoi(peek_argument(0)); //process_lab
...
uint64_t exit_selfie(uint64_t exit_code, char *extras)
{
  // Cleanup framebuffer before exiting
  cleanup_framebuffer();
